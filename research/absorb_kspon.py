...
비어 있던 대화체를 메우려고 쓴 코퍼스다.
🔴 라이선스: CC BY 4.0 이 아니다 — **AI Hub 이용약관**이다(연구·개발 활용은 되지만,
   상업적 이용·제3자 제공은 수행기관과 별도 협의가 필요하다). 그래서 여기서 얻은 수치는
   상업 서비스의 근거로 쓰지 않는다. 대화체 궤적의 정본은 YODAS(CC BY 3.0) 표다
   (server/voice_shape_k2.py · docs/CREDITS.md).
