...
본인 녹음 37개(2,248초)를 `tools/analyze-voice.py`로 재서 얻은 말투 수치(`public/profiles/owner.json`):
음높이 중앙 196Hz(137~280), 4.9음절/초, 구 사이 쉼 320ms, 문장 사이 890ms, 문장 안 하강 −10%, 문장 끝 상승 40%(대화체).
(2026-09-03 F0 추정기를 pYIN으로 바꾼 뒤 값. 이전 219Hz는 25ms 자기상관의 옥타브 편향이 섞여 있었다.)
`synth-supertonic.py`는 이 프로필을 기본으로 쓴다(04번 청취 선택). 음색(목소리 자체)은 `docs/VOICE_CLONING.md`의 제로샷 경로로.
