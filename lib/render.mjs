...
/* 🔴 **전역 하루 상한** — 지출 천장을 고정한다. 이게 없으면 IP 를 바꾸는 만큼 무한히 열려 있다.
   원가 실측(2026-09-13 · research/toolkit/2026-09-13-audio-pipeline.json):
     컨테이너 standard-3 가동 = $0.0000611/초 · 합성 RTF 약 1.0(2 vCPU) · 345자/분
     → 1,000자 ≈ **$0.011** (약 16원) · 오디오 1시간 ≈ $0.22
   20만 자/일 = 약 $2.2/일 → **월 최대 약 $66**. Workers Paid 에 3.1 컨테이너시간(≈6.2만 자)이 포함되므로
   실제 청구는 이보다 작다. 값은 wrangler.jsonc 의 vars.DAILY_CHAR_CAP 로 덮어쓸 수 있다.
   캐시 적중(합성 없는 요청)은 세지 않는다 — handleRender 가 R2 miss 뒤에만 계량한다. */
export const GLOBAL_DAILY_CHARS = 200000;
export const globalCap = (env) => {
  const v = parseInt((env && env.DAILY_CHAR_CAP) || '', 10);
  return Number.isFinite(v) && v > 0 ? v : GLOBAL_DAILY_CHARS;
};
