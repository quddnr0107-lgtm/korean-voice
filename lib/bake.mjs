...
export const BATCH = 4;        // 알람 한 번에 컨테이너 **하나가** R2 로 옮기는 조각 수(조각당 합성 약 11.5초 실측 · 알람 벽시계 1분 안)
export const WARM_AHEAD = 12;  // 컨테이너 하나의 대기열에 앞서 넣어 둘 조각 수(알람이 받을 때 이미 디스크에 있게)
export const SHARDS = 5;       // 🔴 필살기 — 굽기 컨테이너를 여럿 띄운다(bake-0…). 시간은 1/N, 총비용은 같다(2026-09-04 사용자 「더 빨리」)
export const MAX_TRIES = 3;
export const MISMATCH_WAIT_MS = 60000;
export const TICK_MS = 250;
const pad = (n) => String(n).padStart(9, '0');

export function makeBaker({ storage, r2, fetchContainer, recipeTag, now = () => Date.now(), setAlarm, batch = BATCH, warmAhead = WARM_AHEAD, shards = SHARDS }) {
  ...
  async function tick() {
    ...
    const items = await pendingItems(shards * (batch + warmAhead));
    if (!items.length) { await bump((st) => { st.pending = 0; st.finished_at = st.finished_at || now(); }); return { idle: true }; }
    // 조각을 컨테이너(샤드)마다 나눠 **동시에** 굽는다 — 샤드 i 는 자기 몫을 앞서 굽기(/warm)한 뒤 batch 개를 /tts 로 받아 R2 에 넣는다
    // 🔴 샤드 배정을 알람마다 돌린다 — 한 샤드가 못 뜨면(계정 한도·이미지 롤아웃) 같은 조각이 그 샤드에만 계속 걸려 영영 안 구워진다
    const off = Math.floor(now() / 1000) % shards;
    const lanes = Array.from({ length: shards }, () => []);
    items.forEach((it, i) => lanes[(i + off) % shards].push(it));
    const results = await Promise.all(lanes.map((lane, si) => runLane(lane, si)));
    let done = 0, error = 0, mism = 0, skipped = 0, mismatch = false;
    for (const r of results) { done += r.done; error += r.error; mism += r.mism; skipped += r.skipped; mismatch = mismatch || r.mismatch; }
    await bump((st) => { st.done += done + skipped; st.error += error; st.mismatch += mism; st.pending = Math.max(0, st.pending - done - skipped - error); st.last_at = now(); });
    await setAlarm(now() + (mismatch ? MISMATCH_WAIT_MS : TICK_MS));
    return { done, skipped, mismatch };
  }
  async function runLane(lane, si) {
    const out = { done: 0, error: 0, mism: 0, skipped: 0, mismatch: false, down: false };
    if (!lane.length) return out;
    const groups = new Map();
    for (const it of lane) { const g = `${it.v}|${it.s}|${it.r}`; if (!groups.has(g)) groups.set(g, { v: it.v, s: it.s, r: it.r, texts: [] }); groups.get(g).texts.push(it.t); }
    for (const g of groups.values()) {
      try { await fetchContainer('/warm', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(g) }, si); } catch (_) { /* 깨우는 중일 수 있다 */ }
    }
    for (const it of lane.slice(0, batch)) {
      // 다른 길(학생 청취·다른 샤드)이 먼저 넣었으면 굽지 않는다
      if (r2 && await r2.head(it.key).catch(() => null)) { await storage.delete([it.k, 'k:' + it.key]); out.skipped++; continue; }
      let res;
      try { res = await fetchContainer('/tts?v=' + encodeURIComponent(it.v) + '&t=' + encodeURIComponent(it.t) + '&s=' + it.s + '&r=' + it.r, undefined, si); } catch (e) { res = null; }
      // 컨테이너에 못 닿은 것(fetch 가 던짐)은 조각 탓이 아니다 — tries 를 안 올리고 이 샤드는 이번 알람을 쉰다(다음 알람에 다른 샤드로 돌아간다)
      if (res === null) { out.down = true; break; }
      if (res.ok && (res.headers.get('Content-Type') || '').startsWith('audio/')) {
        if ((res.headers.get('X-TTS-Recipe') || '') !== recipeTag) { out.mismatch = true; out.mism++; break; }
        const bytes = new Uint8Array(await res.arrayBuffer());
        await r2.put(it.key, bytes, { httpMetadata: { contentType: 'audio/mpeg', cacheControl: 'public, max-age=31536000, immutable' } });
        await storage.delete([it.k, 'k:' + it.key]); out.done++;
      } else {
        const tries = (it.tries || 0) + 1;
        if (tries >= MAX_TRIES) { await storage.delete([it.k, 'k:' + it.key]); await storage.put('e:' + it.k.slice(2), { ...it, k: undefined, tries, status: res ? res.status : 'fetch_failed' }); out.error++; }
  ...
  /* pending 을 저장소에서 다시 센다 — 집계가 어긋났을 때(3회차처럼) 한 번 맞춘다.
     🔴 겸해서 키 색인(k:)을 되채운다 — 색인이 생기기 전에 들어온 항목은 색인이 없어 재실행 때 두 번 들어간다. */
  async function recount() {
    let n = 0, start, backfilled = 0;
    for (;;) {
      const m = await storage.list({ prefix: 'q:', limit: 1000, ...(start ? { startAfter: start } : {}) });
      if (!m.size) break;
      n += m.size; start = [...m.keys()].pop();
      const entries = [...m.entries()];
      for (let i = 0; i < entries.length; i += 100) {
        const part = entries.slice(i, i + 100);
        const have = await storage.get(part.map(([, v]) => 'k:' + v.key));
        const puts = {}; for (const [k, v] of part) if (!have.has('k:' + v.key)) { puts['k:' + v.key] = k.slice(2); backfilled++; }
        if (Object.keys(puts).length) await storage.put(puts);
      }
      if (m.size < 1000) break;
    }
    await bump((st) => { st.pending = n; });
    return { ...(await status()), backfilled };
  }
