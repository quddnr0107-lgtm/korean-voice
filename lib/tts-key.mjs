...
export const TAGS = {
  ...
  /* 🔴 rev 2 (2026-09-15) — 2026-09-15 굽기에서 **전량 다시 구웠다**(한 샤드 로그 「구움 265 · 실패 0」 =
     건너뛴 것 0 · 그때 `/bake/has` 가 고장 나 있어 아무것도 안 건너뛰었다). 그 커밋(583c5c0)에는
     말끝 죽은 공백을 없애는 `tail_trim` 이 들어 있다.
     R2 키는 `voice|steps|r|벌|글` 이라 **rev 가 안 들어간다** → 덮어써졌다. 그런데 이미 들어 본 사람의
     브라우저에는 `max-age=31536000, immutable` 사본이 박혀 있어 **주소가 안 바뀌면 1년간 옛 소리가 난다.**
     rev 는 **주소에만** 들어가므로 이 한 줄이 그 사본을 흘려보낸다(용량은 안 는다). */
  k2: { steps: 16, rev: 2, label: '지금 목소리' },
};
