...
/* 🔴 **한 요청에서 다 지우지 마라**(2026-09-15) — 5만8천 개를 한 번에 지우려다 러너의 fetch 가
   헤더 타임아웃(5분)으로 끊겼다. 워커는 다 지운 **뒤에야** 응답을 보내므로, 오래 걸리는 만큼
   클라이언트가 먼저 포기한다. 실측 그때 14,500개는 실제로 지워져 있었다 — 「끊겼다」가
   「안 지워졌다」가 아니다. 한 번에 지울 수를 묶고 부르는 쪽이 돌게 만든다. */
export const MAX_DELETE = 6000;

export async function prune({ r2, keep, prefix = 'tts/', dry = true, force = false, listLimit = LIST_LIMIT, maxPages = 400, maxDelete = MAX_DELETE }) {
  ...
  /* 이번 회차에 지울 몫만 자른다. 나머지는 부르는 쪽이 다시 불러 가져간다. */
  const 이번 = maxDelete > 0 ? drop.slice(0, maxDelete) : drop;
  let deleted = 0;
  if (!dry) for (let i = 0; i < 이번.length; i += DELETE_BATCH) { const part = 이번.slice(i, i + DELETE_BATCH); await r2.delete(part); deleted += part.length; }
  /* 🔴 `남음` 은 **이번에 안 지운 것**이다(마른 실행이면 drop 전부). 「0 이면 끝」이 부르는 쪽의 자다. */
  const 남음 = drop.length - deleted;
  return { ok: true, dry, prefix, pages, truncated: !!cursor, total, keep: keepSet.size, hit, drop: drop.length, deleted, 남음, done: 남음 === 0, sample: drop.slice(0, 5) };
}
