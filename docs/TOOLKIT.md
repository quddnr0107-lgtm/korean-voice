...
### 실측 — 끝까지 돈다 (2026-09-13 · Chromium 141 헤드리스)

| 항목 | 값 |
|---|---|
| 모델 적재 | 20.0초 (4개 ONNX · 한 번만, 그 뒤 Cache API) |
| 합성 | 오디오 4.17초를 10.83초에 → **RTF 2.597** (1 스레드 · wasm SIMD · fp32) |
| 실행 제공자 | WebGPU 어댑터는 잡혔으나 컨텍스트 실패 → **wasm 자동 폴백** |
| 표본 | 44.1kHz · 피크 0.84 · 읽기 「이천이십육 년 구 월 삼 일 접수 인원은, 천이백삼십사…」 |

- 레포의 앞선 브라우저 실측(RTF 1.65~2.28)은 **fp16 양자화 모델**이었다. fp32 인 지금이 더 느린 것은 당연하고,
  fp16 으로 바꾸면 **속도와 다운로드 크기(384MB→절반)를 동시에** 얻는다 — 다음 강화 대상.
- RTF > 1 이므로 **실시간 재생에는 부적합**하다. 대본을 받아 파일로 만드는 용도(배치)에 쓰고,
  실시간이 필요하면 서버 경로로 간다(서버 4코어 RTF 0.521).
- 재현: `research/toolkit/browser/` (워커와 같은 헤더로 내주는 서버 + 단계별 계측기).
...
6. **numThreads>1 이면 세션 생성이 영구 대기한다** — ESM 빌드의 pthread 워커가 `importScripts` 에 실패한다
   (1.17 dist 에 `.worker.js` 가 없다). 교차출처 격리는 켜 뒀지만 스레드는 실제 브라우저 확인 뒤에 켠다.
7. **WebGPU 가 깨진 기기에서 세션이 무너진다** — 어댑터를 실제로 요청해 보고, 그래도 실패하면 wasm 으로
   통째로 다시 만든다(헤드리스에서 이 폴백이 실제로 작동했다).
8. **다듬기가 빠져 소리가 달랐다** — `server/voice_shape.py` 의 U5 중 4단계를 `public/voice-shape.mjs` 로
   옮겼다(트림·페이드·첫 음절 보강·꼬리 트림·피크 0.89). Praat PSOLA 억양만 옮기지 못했다.

## ⓪-1 지출 천장 — 전역 하루 상한 (2026-09-13)

무료로 공개하면 개인 한도(IP 당 하루 3,000자)만으로는 **천장이 없다** — IP 를 바꾸는 만큼 열려 있다.
그래서 전역 계량기를 하나 두고 하루 합성량을 세어 막는다.

| 값 | 근거 |
|---|---|
| 기본 상한 **20만 자/일** | 원가 실측 1,000자 ≈ $0.011 → 약 **$2.2/일 · 월 최대 약 $66** |
| 조정 | `wrangler.jsonc` 의 `vars.DAILY_CHAR_CAP` (배포로 반영) |
| 세는 시점 | **R2 적중은 세지 않는다** — 합성이 없으면 비용이 없다 |
| 순서 | 전역을 먼저 **보고**(세지 않고) → 개인 한도를 세고 → 전역을 센다. 전역이 찼으면 개인 한도를 깎지 않는다 |
| 구현 | 계량기와 **같은 BakeQueue 클래스의 오브젝트 하나**(`meter|global`). 새 DO 클래스는 배포가 거부된다(이 저장소에서 실제로 당했다) |
| 소진되면 | 서버 경로는 429, **브라우저 경로는 계속 된다**(우리 비용 0이므로 막을 이유가 없다) |

`test/render-cap.test.mjs` 6개가 워커 코드를 그대로 돌려 이 동작을 잰다.
