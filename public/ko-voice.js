...
(function (root, factory) {
  ...
  const RANGE_PREDICATE = /^(.+?)(입니다|이에요|이었다|이라면|이다|예요|였다|이며|이고|이지|이야|이면|라면|까지|으로|로|이|가|은|는|을|를|에|의|과|와)$/;
  ...
  function normalize(text) {
    ...
    t = t.replace(/\b(?:K-?(?:1A1|2A1|9A1|21|1|2|9)|KF-?21|FA-?50|F-?35A)\b/gi, (m) => MILITARY_MODEL[m.toUpperCase()] || m);
    ...
    // 숫자 앞 단독 물결표는 범위가 아니라 근사치다: ~97점 → 약 97점. 이항 범위의 가운데 물결표에는 걸리지 않는다.
    t = t.replace(/(^|[\s([,:])\s*[~∼～]\s*(?=\d)/g, '$1약 ');
    // 날짜 범위: 2026-09-11~2026-09-12 → ...부터 ...까지 (수량 범위의 '에서'와 분리)
    t = t.replace(/\b(\d{4})\s*[.\-/]\s*(\d{1,2})\s*[.\-/]\s*(\d{1,2})\.?\s*[~∼～]\s*(\d{4})\s*[.\-/]\s*(\d{1,2})\s*[.\-/]\s*(\d{1,2})\.?(?!\d)(?:(까지(?:는|도|만)?|에(?:는|도|만)?)(?=\s|[,.!?)]|$))?/g,
    ...
    t = t.replace(/\b(\d{4})\s*년\s*(\d{1,2})\s*월\s*(\d{1,2})\s*일?\s*[~∼～]\s*(\d{4})\s*년\s*(\d{1,2})\s*월\s*(\d{1,2})\s*일?(?!\d)(?:(까지(?:는|도|만)?|에(?:는|도|만)?)(?=\s|[,.!?)]|$))?/g,
      (m, y1, mo1, d1, y2, mo2, d2, tail) => {
        const focus = tail && tail.match(/(?:는|도|만)$/);
        return readSino(y1) + ' 년 ' + readWithUnit(mo1, '월') + ' ' + readSino(d1) + ' 일부터 ' + readSino(y2) + ' 년 ' + readWithUnit(mo2, '월') + ' ' + readSino(d2) + ' 일까지' + (focus ? focus[0] : '');
      });
  ...
  // 다음 어절이 '수사 한 덩어리'일 때만 범위로 본다. 복합 고유어 수(스물여덟·서른두)도 한 덩어리다.
  const NUM_WORD = /^(?:[영일이삼사오육칠팔구십백천만억조]+|시월|한|두|세|네|다섯|여섯|일곱|여덟|아홉|스무|(?:열|스물|서른|마흔|쉰|예순|일흔|여든|아흔)(?:한|두|세|네|다섯|여섯|일곱|여덟|아홉)?)$/;
  const NUM_HEAD = /^(?:시월|[영일이삼사오육칠팔구십백천만억조]|한|두|세|네|다섯|여섯|일곱|여덟|아홉|열|스무|스물|서른|마흔|쉰|예순|일흔|여든|아흔)/;
  const NUM_PREFIX = /^(?:시월|[영일이삼사오육칠팔구십백천만억조]+|스무|(?:열|스물|서른|마흔|쉰|예순|일흔|여든|아흔)(?:한|두|세|네|다섯|여섯|일곱|여덟|아홉)?|한|두|세|네|다섯|여섯|일곱|여덟|아홉)/;
