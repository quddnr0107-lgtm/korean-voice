/* Supertonic 3 브라우저 추론 코어 — supertone-inc/supertonic 의 web/helper.js 를 그대로 가져왔다.
   출처: https://github.com/supertone-inc/supertonic (web/ 예제 · MIT) · 가져온 날 2026-09-13
   라이선스: 샘플 코드는 MIT. **모델 가중치는 BigScience OpenRAIL-M** 이고 사용 제한이 하위 이용자에게
             승계된다 — docs/CREDITS.md 와 docs/TERMS-SNIPPET.md 가 정본이다.
   우리가 바꾼 것: 이 머리말과 **import 한 줄**(`'onnxruntime-web'` → `'./ort-cdn.js'`). 바 스펙을 쓰면
             import map 이 필요하고, import map 은 인라인 스크립트라서 CSP 에 'unsafe-inline' 을 켜야 한다.
   🔴 이 파일은 손으로 고치지 않는다. 고칠 것이 생기면 우리 쪽(public/local-tts.js)에서 감싼다. */
import * as ort from './ort-cdn.js';   // (우리가 바꾼 한 줄 · 아래 머리말 참고)
