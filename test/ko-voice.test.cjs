...
test('숫자·단위 정규화 — 고유어/한자어·달·큰 수·시각·범위', () => {
  const cases = [
    ...
    ['2026-09-11~2026-09-12', '이천이십육 년 구 월 십일 일부터 이천이십육 년 구 월 십이 일까지'],
    ...
    ['12.5%', '십이 점 오 퍼센트'], ['0.5배', '영 점 오 배'], ['5km', '오 킬로미터'], ['-3도', '영하 삼 도'], ['영하 -3도', '영하 삼 도'],
    ['-3~0도', '영하 삼 도에서 영 도'], ['-3~-1도', '영하 삼 도에서 영하 일 도'],
...
// ─── 범위·불릿 정확도(대량 corpus 감사에서 나온 실제 결함) ───
const 낭독 = (t) => K.prepare(t).sentences.map((s) => K.joinSpokenChunks(s.chunks)).join(' ⏐ ');

test('범위 뒤 서술어·조사는 단위가 아니다 — 앞쪽 항에 복제되지 않는다', () => {
  assert.strictEqual(낭독('복무기간은 18~21개월이다.'), '복무기간은 십팔 개월에서 이십일 개월이다.');
  assert.strictEqual(낭독('실측 지연이 1~3시간이다.'), '실측 지연이 한 시간에서 세 시간이다.');
  assert.strictEqual(낭독('보통 3~6문장으로 답한다.'), '보통 삼문장에서 육문장으로, 답한다.');
  assert.strictEqual(낭독('훈련은 2025~2026년에 시행한다.'), '훈련은 이천이십오 년에서 이천이십육 년에, 시행한다.');
  // 복합 단위는 그대로 양쪽에 붙는다(1~4년차·4~5만원).
  assert.strictEqual(낭독('1~4년차 동원훈련 대상이다.'), '일 년차에서 사 년차 동원훈련 대상이다.');
  assert.strictEqual(낭독('토익 응시료 4~5만원이 든다.'), '토익 응시료 사만 원에서 오만 원이 든다.');
});

test('범위 한가운데는 끊지 않는다 — 다른 곳의 쉼은 그대로 둔다', () => {
  const o = 낭독('모든 수치는 2025~2026년 기준이며 회차·부대 상황에 따라 달라집니다.');
  assert.ok(/이천이십오 년에서 이천이십육 년/.test(o), '범위가 한 덩어리로 붙는다: ' + o);
  // 진짜 장소격 '에서' 뒤 호흡은 살아 있어야 한다(범위로 오인 금지).
  assert.ok(/세션에서, 이미/.test(낭독('이번 세션에서 이미 확인한 환경 사실부터 뒤져야 한다는 점을 잊지 마세요.')),
    '수사가 아닌 어절 뒤에서는 평소대로 끊는다');
  // 수량과 단위 사이도 끊지 않는다.
  assert.ok(!/오만,\s*원/.test(낭독('토익 응시료 4~5만원이 든다.')), '수량과 단위 사이를 끊지 않는다');
});

test('멱등성 — 앞머리 따옴표·주석기호가 붙어도 불릿은 한 번에 지워진다', () => {
  for (const t of ['"- 예비군 훈련은 해마다 받습니다.', '“- 훈련은 받습니다.', '// - 운영자 인증 필수.', '- 훈련은 받습니다.']) {
    const a = K.normalize(t);
    assert.strictEqual(a, K.normalize(a), '두 번 돌려도 같아야 한다: ' + JSON.stringify(t) + ' → ' + JSON.stringify(a));
...
test('날짜 범위 뒤 조사는 생성된 까지와 자연스럽게 합친다', () => {
